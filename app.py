...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    stats_bar_html = f"""
<div style='background:var(--surface);border:1px solid var(--border);border-left:3px solid {color};border-radius:var(--r-md);padding:1.1rem 1.5rem;box-shadow:var(--shadow-sm);margin-bottom:1.2rem'>

    <div style='display:flex;align-items:center;flex-wrap:wrap;gap:0.4rem;margin-bottom:0.85rem'>
        <span style='font-weight:700;color:{color};font-size:0.9rem;font-family:"DM Mono",monospace'>Rank #{rank} of {len(scores)}</span>
        <span style='color:var(--border-2);font-size:0.9rem;margin:0 0.2rem'>&nbsp;/&nbsp;</span>
        <span style='font-size:0.82rem;color:var(--text-2);font-family:"DM Mono",monospace'>WCI <b style='color:{color}'>{wci:.1f}</b>&nbsp;/&nbsp; peer avg <b>{peer_avg_wci:.1f}</b>&nbsp;/&nbsp; range [{wci_min_all:.1f} - {wci_max_all:.1f}]</span>
        {caveat_note}
    </div>

    <div style='display:flex;align-items:center;gap:0.75rem;margin-bottom:0.85rem'>
        <span style='font-size:0.62rem;color:var(--text-3);font-family:"DM Mono",monospace;width:16px'>0</span>
        <div style='flex:1;background:rgba(5,150,105,0.08);border-radius:999px;height:7px;overflow:hidden;border:1px solid rgba(5,150,105,0.12);position:relative'>
            <div style='position:absolute;left:{_peer_tick}%;top:0;bottom:0;width:1.5px;background:rgba(5,150,105,0.35)'></div>
            <div style='background:linear-gradient(90deg,{color},{color}88);width:{wci_pct}%;height:7px;border-radius:999px'></div>
        </div>
        <span style='font-size:0.62rem;color:var(--text-3);font-family:"DM Mono",monospace;width:24px'>100</span>
    </div>

    <div style='display:flex;flex-wrap:wrap;gap:1.6rem'>
        <div style='font-family:"DM Mono",monospace;font-size:0.72rem'>
            <span style='color:var(--text-3)'>Avg Rating</span><br>
            <span style='color:#d97706;font-weight:600'>{avg_rat:.2f} stars</span>
            <span style='color:var(--text-3)'> / peer avg {peer_avg_rating:.2f}</span>
        </div>
        <div style='font-family:"DM Mono",monospace;font-size:0.72rem'>
            <span style='color:var(--text-3)'>Like Sentiment</span><br>
            <span style='color:#059669;font-weight:600'>{like_mean:+.4f}</span>
        </div>
        <div style='font-family:"DM Mono",monospace;font-size:0.72rem'>
            <span style='color:var(--text-3)'>Dislike Sentiment</span><br>
            <span style='color:#dc2626;font-weight:600'>{dislike_mean:+.4f}</span>
        </div>
        <div style='font-family:"DM Mono",monospace;font-size:0.72rem'>
            <span style='color:var(--text-3)'>Combined Sentiment</span><br>
            <span style='color:{color};font-weight:600'>{combined_mean:+.4f}</span>
        </div>
        <div style='font-family:"DM Mono",monospace;font-size:0.72rem'>
            <span style='color:var(--text-3)'>Reviews</span><br>
            <span style='color:var(--text);font-weight:600'>{_n_comp:,}</span>
            <span style='color:var(--text-3)'> ({pct_of_dataset:.1f}% of dataset)</span>
        </div>
    </div>
...
"""
    ...
    with phrase_col:
        ...
        for d in kw_data:
            ...
            # Build positive keyword bars
            pos_bars_html = ""
            for kw, cnt in d["top_pos"]:
                if cnt == 0:
                    continue
                bar_w = round(cnt / global_pos_max * 100)
                pos_bars_html += f"""
<div style='display:flex;align-items:center;gap:0.45rem;margin-bottom:0.28rem'>
    <span style='font-size:0.67rem;font-family:DM Mono,monospace;color:#059669;width:74px;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{kw}</span>
    <div style='flex:1;background:rgba(5,150,105,0.08);border-radius:999px;height:5px;overflow:hidden;min-width:40px'>
        <div style='background:#059669;width:{bar_w}%;height:5px;border-radius:999px;opacity:0.82'></div>
    </div>
    <span style='font-size:0.63rem;color:#9ca3af;font-family:DM Mono,monospace;width:20px;text-align:right;flex-shrink:0'>{cnt}</span>
</div>"""

            # Build negative keyword bars
            neg_bars_html = ""
            for kw, cnt in d["top_neg"]:
                if cnt == 0:
                    continue
                bar_w = round(cnt / global_neg_max * 100)
                neg_bars_html += f"""
<div style='display:flex;align-items:center;gap:0.45rem;margin-bottom:0.28rem'>
    <span style='font-size:0.67rem;font-family:DM Mono,monospace;color:#dc2626;width:74px;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{kw}</span>
    <div style='flex:1;background:rgba(220,38,38,0.08);border-radius:999px;height:5px;overflow:hidden;min-width:40px'>
        <div style='background:#dc2626;width:{bar_w}%;height:5px;border-radius:999px;opacity:0.82'></div>
    </div>
    <span style='font-size:0.63rem;color:#9ca3af;font-family:DM Mono,monospace;width:20px;text-align:right;flex-shrink:0'>{cnt}</span>
</div>"""

            no_data = "<span style='font-size:0.67rem;color:#9ca3af;font-family:DM Mono,monospace'>no data</span>"
            if not pos_bars_html:
                pos_bars_html = no_data
            if not neg_bars_html:
                neg_bars_html = no_data

            card_html = f"""
<div style='background:var(--surface);border:1px solid var(--border);border-left:3px solid {meta_k["color"]};border-radius:var(--r-md);padding:0.75rem 0.9rem;margin-bottom:0.55rem;box-shadow:var(--shadow-sm)'>

    <div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:0.6rem'>
        <span style='font-size:0.75rem;font-weight:600;color:{meta_k["color"]};font-family:DM Sans,sans-serif'>{meta_k["icon"]} {d["label"]}</span>
        <div style='display:flex;align-items:center;gap:0.5rem'>
            <span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.08em;color:var(--text-3);font-family:DM Mono,monospace'>Net</span>
            <span style='font-size:0.8rem;font-weight:700;color:{net_color};font-family:DM Mono,monospace'>{net_label}</span>
        </div>
    </div>

    <div style='display:grid;grid-template-columns:1fr 1fr;gap:0.65rem'>
        <div>
            <div style='font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:#059669;font-family:DM Mono,monospace;margin-bottom:0.35rem;display:flex;align-items:center;gap:0.3rem'>
                <span style='width:6px;height:6px;border-radius:50%;background:#059669;display:inline-block'></span>
                Positive &middot; {d["total_pos"]}
            </div>
            {pos_bars_html}
        </div>
        <div>
            <div style='font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:#dc2626;font-family:DM Mono,monospace;margin-bottom:0.35rem;display:flex;align-items:center;gap:0.3rem'>
                <span style='width:6px;height:6px;border-radius:50%;background:#dc2626;display:inline-block'></span>
                Negative &middot; {d["total_neg"]}
            </div>
            {neg_bars_html}
        </div>
    </div>
        ...
        for col, icon, title, rows in signal_data:
            with col:
                ...
                for lbl, val, clr, sub in rows:
                    sub_html = (
                        f"<span style='font-size:0.65rem;color:var(--text-3);margin-left:0.3rem'>{sub}</span>"
                    ) if sub else ""
                    rows_html += f"""
<div style='display:flex;justify-content:space-between;align-items:baseline;padding:0.38rem 0;border-bottom:1px solid rgba(5,150,105,0.06)'>
    <span style='font-size:0.74rem;color:var(--text-3);font-family:"DM Sans",sans-serif'>{lbl}</span>
    <div style='text-align:right'>
        <span style='font-size:0.78rem;font-weight:600;color:{clr};font-family:"DM Mono",monospace'>{val}</span>
        {sub_html}
    </div>
</div>"""
                card_html = f"""
<div class='ins-card' style='border-top:2px solid rgba(5,150,105,0.2)'>
    <div style='display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem'>
        <div style='width:30px;height:30px;border-radius:8px;background:rgba(5,150,105,0.1);border:1px solid rgba(5,150,105,0.2);display:flex;align-items:center;justify-content:center;font-size:0.95rem'>{icon}</div>
        <span style='font-weight:600;color:var(--text);font-size:0.85rem;font-family:"DM Sans",sans-serif'>{title}</span>
    </div>
    {rows_html}
</div>
"""
