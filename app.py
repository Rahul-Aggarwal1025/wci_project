...
RANK_EMOJI   = {1:"🥇", 2:"🥈", 3:"🥉", 4:"4️⃣", 5:"5️⃣", 6:"6️⃣"}
...
THEME_META = {
    'theme_respect':      {'icon':'⚖️','label':'Respect & Fairness',    'weight':0.28,'color':'#059669'},
    ...
    'theme_wlb':          {'icon':'🏖️','label':'Work-Life Balance',      'weight':0.12,'color':'#ea580c'},
}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    verdict_html = (
        ...
        f"font-family:\"DM Sans\",sans-serif'>{emoji} Verdict: {vname}{caveat}</span>"
    ...
    with col_r:
        ...
        for label, key, clr in [
            ...
            (f"⚖️ Combined (WCI)",   "combined_sentiment", color),
        ]:
            val  = sent_row[key]
            bpct = bar_pct(val)
        ...
        for col, icon, title, rows in signal_data:
            with col:
                ...
                card_html = (
                    ...
                    "<div style='width:30px;height:30px;border-radius:8px;background:rgba(5,150,105,0.1);"
                    "border:1px solid rgba(5,150,105,0.2);display:flex;align-items:center;"
                    "justify-content:center;font-size:0.95rem'>" + icon + "</div>"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with sent_col:
        ...
        for label, key in [
            ...
            ("⚖️ Combined (WCI)", "combined_sentiment"),
        ]:
            va   = sent_a[key]
            vb   = sent_b[key]
            pa   = bar_pct(va)
