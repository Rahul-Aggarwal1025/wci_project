...
if page == "◈  Home & Leaderboard":
    ...
    with left:
        ...
        _veljan_n  = int(len(df[df["company"] == "Veljan Denison"]))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Hero metric cards ───────────────────────────────────────────────────
    # Use actual row count from df (comp_all) for % of dataset — consistent with
    # the Data Quality section which also uses comp_all as denominator/numerator.
    pct_of_dataset = len(comp_all) / len(df) * 100
    # Usable reviews: rows NOT excluded by structured_text OR low_signal
    # Boolean OR mask avoids double-counting rows that satisfy both exclusion criteria.
    # This must match the n_usable formula in the Data Quality section exactly.
    if "structured_text" in comp_all.columns and "low_signal" in comp_all.columns:
        _excl = comp_all["structured_text"].astype(bool) | comp_all["low_signal"].astype(bool)
        usable_reviews = int((~_excl).sum())
    else:
        usable_reviews = len(comp_free)
    ...
    # ── Compact stats bar — rank + WCI progress bar, no editorial verdict ──
    wci_pct = int(round(wci))
    wci_min_all = float(scores["wci_score"].min())
    wci_max_all = float(scores["wci_score"].max())
    peer_avg_wci = float(scores["wci_score"].mean())
    peer_avg_rating = float(scores["avg_rating"].mean())
    like_mean  = float(comp_free["like_sentiment"].mean()) if len(comp_free) > 0 else 0.0
    dislike_mean = float(comp_free["dislike_sentiment"].mean()) if len(comp_free) > 0 else 0.0
    combined_mean = float(comp_free["combined_sentiment"].mean()) if len(comp_free) > 0 else 0.0
    caveat_note = (
        f"<span style='font-size:0.72rem;color:#c49a1a;margin-left:0.75rem'>"
        f"⚠️ Only {n_reviews} reviews — interpret with caution.</span>"
        if sel == "Veljan Denison" else ""
    )

    _peer_tick = int(round(peer_avg_wci))
    _n_comp    = len(comp_all)
    stats_bar_html = (
        f"<div style='background:var(--surface);border:1px solid var(--border);border-left:3px solid {color};border-radius:var(--r-md);padding:1.1rem 1.5rem;box-shadow:var(--shadow-sm);margin-bottom:1.2rem'>"
        f"<div style='display:flex;align-items:center;flex-wrap:wrap;gap:0.4rem;margin-bottom:0.85rem'>"
        f"<span style='font-weight:700;color:{color};font-size:0.9rem;font-family:\"DM Mono\",monospace'>Rank #{rank} of {len(scores)}</span>"
        f"<span style='color:var(--border-2);font-size:0.9rem;margin:0 0.2rem'>&nbsp;/&nbsp;</span>"
        f"<span style='font-size:0.82rem;color:var(--text-2);font-family:\"DM Mono\",monospace'>"
        f"WCI <b style='color:{color}'>{wci:.1f}</b>"
        f"&nbsp;/&nbsp; peer avg <b>{peer_avg_wci:.1f}</b>"
        f"&nbsp;/&nbsp; range [{wci_min_all:.1f} - {wci_max_all:.1f}]"
        f"</span>"
        f"{caveat_note}"
        f"</div>"
        f"<div style='display:flex;align-items:center;gap:0.75rem;margin-bottom:0.85rem'>"
        f"<span style='font-size:0.62rem;color:var(--text-3);font-family:\"DM Mono\",monospace;width:16px'>0</span>"
        f"<div style='flex:1;background:rgba(5,150,105,0.08);border-radius:999px;height:7px;overflow:hidden;border:1px solid rgba(5,150,105,0.12);position:relative'>"
        f"<div style='position:absolute;left:{_peer_tick}%;top:0;bottom:0;width:1.5px;background:rgba(5,150,105,0.35)'></div>"
        f"<div style='background:linear-gradient(90deg,{color},{color}88);width:{wci_pct}%;height:7px;border-radius:999px'></div>"
        f"</div>"
        f"<span style='font-size:0.62rem;color:var(--text-3);font-family:\"DM Mono\",monospace;width:24px'>100</span>"
        f"</div>"
        f"<div style='display:flex;flex-wrap:wrap;gap:1.6rem'>"
        f"<div style='font-family:\"DM Mono\",monospace;font-size:0.72rem'>"
        f"<span style='color:var(--text-3)'>Avg Rating</span><br>"
        f"<span style='color:#d97706;font-weight:600'>{avg_rat:.2f} stars</span>"
        f"<span style='color:var(--text-3)'> / peer avg {peer_avg_rating:.2f}</span>"
        f"</div>"
        f"<div style='font-family:\"DM Mono\",monospace;font-size:0.72rem'>"
        f"<span style='color:var(--text-3)'>Like Sentiment</span><br>"
        f"<span style='color:#059669;font-weight:600'>{like_mean:+.4f}</span>"
        f"</div>"
        f"<div style='font-family:\"DM Mono\",monospace;font-size:0.72rem'>"
        f"<span style='color:var(--text-3)'>Dislike Sentiment</span><br>"
        f"<span style='color:#dc2626;font-weight:600'>{dislike_mean:+.4f}</span>"
        f"</div>"
        f"<div style='font-family:\"DM Mono\",monospace;font-size:0.72rem'>"
        f"<span style='color:var(--text-3)'>Combined Sentiment</span><br>"
        f"<span style='color:{color};font-weight:600'>{combined_mean:+.4f}</span>"
        f"</div>"
        f"<div style='font-family:\"DM Mono\",monospace;font-size:0.72rem'>"
        f"<span style='color:var(--text-3)'>Reviews</span><br>"
        f"<span style='color:var(--text);font-weight:600'>{_n_comp:,}</span>"
        f"<span style='color:var(--text-3)'> ({pct_of_dataset:.1f}% of dataset)</span>"
        f"</div>"
        f"</div>"
        f"</div>"
    )
    ...
    # ── WCI Theme Decomposition — compact number table ──────────────────────
    if sel in all_means.index:
        ...
        contribs_narr = {t: all_means.loc[sel, t] * fw for t, fw in zip(THEME_COLS, fw_arr_narr)}
        ...
        # Build a compact 5-column data strip
        decomp_cells = ""
        for t, fw in zip(THEME_COLS, fw_arr_narr):
            meta   = THEME_META[t]
            raw    = all_means.loc[sel, t]
            contrib = contribs_narr[t]
            share  = abs(contrib) / total_abs * 100
            vs_avg = raw - peer_avg_narr[t]
            clr    = "#059669" if contrib >= 0 else "#dc2626"
            vs_clr = "#059669" if vs_avg >= 0 else "#dc2626"
            decomp_cells += f"""
            <td style='padding:0.7rem 0.9rem;border-right:1px solid var(--border);vertical-align:top'>
                <div style='font-size:0.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:{meta["color"]};font-family:"DM Mono",monospace;margin-bottom:0.4rem'>
                    {meta["icon"]} {meta["label"]}</div>
                <div style='font-family:"DM Serif Display",serif;font-size:1.3rem;color:{clr};line-height:1.1'>{contrib:+.4f}</div>
                <div style='font-size:0.65rem;color:var(--text-3);font-family:"DM Mono",monospace;margin-top:0.25rem'>
                    raw {raw:+.4f} · w {fw:.4f}</div>
                <div style='font-size:0.65rem;font-family:"DM Mono",monospace;margin-top:0.1rem'>
                    <span style='color:var(--text-3)'>share</span>
                    <span style='color:{clr};font-weight:600'> {share:.0f}%</span>
                    &nbsp;·&nbsp;
                    <span style='color:var(--text-3)'>vs avg</span>
                    <span style='color:{vs_clr};font-weight:600'> {vs_avg:+.4f}</span>
                </div>
            </td>"""

        st.markdown(f"""
        <div style='border:1px solid var(--border);border-radius:var(--r-md);background:var(--surface);box-shadow:var(--shadow-sm);overflow-x:auto;margin-bottom:1.2rem'>
            <div style='padding:0.7rem 0.9rem;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center'>
                <span style='font-size:0.68rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:var(--emerald);font-family:"DM Mono",monospace'>◈ WCI Decomposition</span>
                <span style='font-size:0.7rem;color:var(--text-3);font-family:"DM Mono",monospace'>
                    {themes_above}/5 themes above peer avg
                </span>
            </div>
    ...
    # ── Top / Bottom Theme by Contribution ────────────────────────────────
    if sel in all_means.index:
        ...
        for t, fw, meta in zip(THEME_COLS, fw_arr_wsk, [THEME_META[t] for t in THEME_COLS]):
            wsk_contribs.append({
                "theme": meta["label"], "icon": meta["icon"], "col": t,
        ...
        peer_avg_narr_wsk = all_means.mean()

        wsk_col_l, wsk_col_r = st.columns(2, gap="large")
        with wsk_col_l:
            tc = saving_tc
            vs_avg = tc["raw"] - peer_avg_narr_wsk[tc["col"]]
            st.markdown(f"""
            <div style='background:var(--surface);border:1px solid rgba(5,150,105,0.25);border-left:4px solid #1a8a4a;border-radius:var(--r-md);padding:1.1rem 1.4rem;box-shadow:var(--shadow-sm);height:100%'>
                <div style='font-size:0.63rem;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:#1a8a4a;margin-bottom:0.55rem;font-family:"DM Mono",monospace'>✦ Highest Contributing Theme</div>
                <div style='font-family:"DM Serif Display",serif;font-size:1.35rem;color:#1a8a4a;line-height:1.1;margin-bottom:0.55rem'>
                    {tc['icon']} {tc['theme']}</div>
                <div style='display:grid;grid-template-columns:repeat(3,1fr);gap:0.8rem;font-family:"DM Mono",monospace;font-size:0.7rem'>
                    <div><span style='color:var(--text-3)'>Raw score</span><br>
                        <span style='color:#1a8a4a;font-weight:700'>{tc['raw']:+.4f}</span></div>
                    <div><span style='color:var(--text-3)'>Weight</span><br>
                        <span style='color:#1a8a4a;font-weight:700'>{tc['weight']:.4f}</span></div>
                    <div><span style='color:var(--text-3)'>Contribution</span><br>
                        <span style='color:#1a8a4a;font-weight:700'>{tc['contribution']:+.4f}</span></div>
                    <div><span style='color:var(--text-3)'>vs Peer Avg</span><br>
                        <span style='color:{"#059669" if vs_avg>=0 else "#dc2626"};font-weight:700'>{vs_avg:+.4f}</span></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with wsk_col_r:
            ...
            vs_avg = tc["raw"] - peer_avg_narr_wsk[tc["col"]]
            st.markdown(f"""
            <div style='background:var(--surface);border:1px solid rgba(220,38,38,0.22);border-left:4px solid #ef4444;border-radius:var(--r-md);padding:1.1rem 1.4rem;box-shadow:var(--shadow-sm);height:100%'>
                <div style='font-size:0.63rem;font-weight:700;text-transform:uppercase;letter-spacing:0.12em;color:#ef4444;margin-bottom:0.55rem;font-family:"DM Mono",monospace'>⚑ Lowest Contributing Theme</div>
                <div style='font-family:"DM Serif Display",serif;font-size:1.35rem;color:#ef4444;line-height:1.1;margin-bottom:0.55rem'>
                    {tc['icon']} {tc['theme']}</div>
                <div style='display:grid;grid-template-columns:repeat(3,1fr);gap:0.8rem;font-family:"DM Mono",monospace;font-size:0.7rem'>
                    <div><span style='color:var(--text-3)'>Raw score</span><br>
                        <span style='color:#ef4444;font-weight:700'>{tc['raw']:+.4f}</span></div>
                    <div><span style='color:var(--text-3)'>Weight</span><br>
                        <span style='color:#ef4444;font-weight:700'>{tc['weight']:.4f}</span></div>
                    <div><span style='color:var(--text-3)'>Contribution</span><br>
                        <span style='color:#ef4444;font-weight:700'>{tc['contribution']:+.4f}</span></div>
                    <div><span style='color:var(--text-3)'>vs Peer Avg</span><br>
                        <span style='color:{"#059669" if vs_avg>=0 else "#dc2626"};font-weight:700'>{vs_avg:+.4f}</span></div>
                </div>
            </div>
        ...
        for label, key, clr in [
            ...
            <div style='margin-bottom:1.3rem'>
                <div style='display:flex;justify-content:space-between;margin-bottom:0.32rem'>
                    <span style='font-size:0.82rem;color:var(--text-2);font-weight:500;font-family:"DM Sans",sans-serif'>{label}</span>
                    <span style='font-size:0.88rem;font-weight:400;color:{clr};font-family:"DM Serif Display",serif;letter-spacing:-0.01em'>{val:+.3f}</span>
                </div>
                <div style='background:rgba(5,150,105,0.05);border-radius:999px;height:10px;overflow:hidden;border:1px solid rgba(5,150,105,0.08);position:relative'>
                    <div style='position:absolute;left:{midpoint:.1f}%;top:0;bottom:0;width:1px;background:rgba(5,150,105,0.25)'></div>
                    <div style='background:linear-gradient(90deg,{clr}99,{clr});width:{bpct_disp:.1f}%;height:10px;border-radius:999px;margin-left:{(midpoint - bpct_disp) if val < 0 else midpoint if bpct_disp < midpoint else 0:.0f}%;opacity:0.88'></div>
                </div>
                <div style='display:flex;justify-content:space-between;font-size:0.62rem;color:var(--text-3);margin-top:0.15rem;font-family:"DM Mono",monospace'>
    ...
    st.markdown("""
    <div style='font-size:0.8rem;color:var(--text-3);margin-bottom:1rem;font-family:"DM Sans",sans-serif;font-weight:300;max-width:640px;line-height:1.7'>
        ...
        with tc2:
            ...
            for i, tc in enumerate(sorted(theme_contribs, key=lambda x: x["contribution"], reverse=True)):
                ...
                <div class='signal-card'>
                    <div style='display:flex;justify-content:space-between;align-items:center'>
                        <span style='font-size:0.81rem;font-weight:500;color:var(--text);font-family:"DM Sans",sans-serif'>
                            {tc['icon']} {tc['theme']}</span>
                        <span style='font-size:0.76rem;font-weight:600;color:{vs_clr};font-family:"DM Mono",monospace'>{vs_txt} vs avg</span>
                    </div>
                    <div style='font-size:0.68rem;color:var(--text-3);margin-top:0.2rem;font-family:"DM Mono",monospace'>
        ...
        for i, (cn, sl_lbl) in enumerate(zip(THEME_COLS, THEME_SHORT)):
            ...
            with tb_cols[i]:
                ...
                <div class='stat-box' style='border-top:2px solid {meta["color"]}44'>
                    ...
                    <div style='background:rgba(5,150,105,0.05);border-radius:999px;height:4px;overflow:hidden;margin:0.45rem 0;border:1px solid {meta["color"]}20'>
                        <div style='background:{clr};width:{bpct_v:.0f}%;height:4px;border-radius:999px;opacity:0.85'></div>
                    </div>
                    <div style='background:rgba(5,150,105,0.03);border-radius:999px;height:3px;overflow:hidden;margin-bottom:0.45rem'>
                        <div style='background:rgba(5,150,105,0.3);width:{bpct_a:.0f}%;height:3px;border-radius:999px'></div>
                    </div>
                    <span style='font-size:0.67rem;color:{clr};font-weight:600;font-family:"DM Mono",monospace'>
    ...
    with dist_col:
        ...
        for star in [5, 4, 3, 2, 1]:
            ...
            <div style='display:flex;align-items:center;gap:0.6rem;margin-bottom:0.55rem'>
                <span style='font-size:0.72rem;color:{clr};font-family:"DM Mono",monospace;width:72px;flex-shrink:0;letter-spacing:0.05em'>{stars_str}</span>
                <div style='flex:1;background:rgba(5,150,105,0.05);border-radius:999px;height:8px;overflow:hidden;border:1px solid rgba(5,150,105,0.07)'>
                    <div style='background:{clr};width:{pct:.0f}%;height:8px;border-radius:999px;opacity:0.82'></div>
                </div>
                <span style='font-size:0.68rem;color:var(--text-3);width:58px;text-align:right;font-family:"DM Mono",monospace'>{cnt} ({pct:.0f}%)</span>
            </div>""", unsafe_allow_html=True)
        ...
        if len(comp_all) >= 5:
            ...
            st.markdown(f"""
            <div style='margin-top:0.85rem;font-size:0.72rem;color:var(--text-3);font-family:"DM Mono",monospace'>
    ...
    with trend_col:
        ...
        # ── Compute full date range for this company ────────────────────────
        _full_dates = comp_free["review_date"].dropna()
        _date_min   = _full_dates.min()
        _date_max   = _full_dates.max()
        _year_min   = int(_date_min.year) if pd.notna(_date_min) else 2019
        _year_max   = int(_date_max.year) if pd.notna(_date_max) else 2024

        # ── Compact control row ─────────────────────────────────────────────
        tc0, tc1, tc2, tc3 = st.columns([1.6, 1, 1, 1.4])

        with tc0:
            _preset = st.selectbox(
                "Quick range",
                ["All Time", "Last 1 Year", "Last 2 Years", "Last 3 Years", "Custom"],
                key=f"trend_preset_{sel}",
                label_visibility="collapsed",
            )

        with tc1:
            _granularity = st.selectbox(
                "Granularity",
                ["Quarterly", "Monthly", "Half-Yearly", "Yearly"],
                key=f"trend_gran_{sel}",
                label_visibility="collapsed",
            )
        ...
        with tc2:
            _overlay = st.selectbox(
                "Overlay",
                ["Avg Rating", "Review Volume", "None"],
                key=f"trend_overlay_{sel}",
                label_visibility="collapsed",
            )

        # Preset → year bounds
        if _preset == "All Time":
            _yr_from, _yr_to = _year_min, _year_max
        elif _preset == "Last 1 Year":
            _yr_from, _yr_to = max(_year_min, _year_max - 1), _year_max
        elif _preset == "Last 2 Years":
            _yr_from, _yr_to = max(_year_min, _year_max - 2), _year_max
        elif _preset == "Last 3 Years":
            _yr_from, _yr_to = max(_year_min, _year_max - 3), _year_max
        else:
            _yr_from, _yr_to = _year_min, _year_max  # will be overridden below
        ...
        with tc3:
            if _preset == "Custom":
                _custom_range = st.slider(
                    ...
                    key=f"trend_slider_{sel}",
                    label_visibility="collapsed",
                )
                _yr_from, _yr_to = _custom_range
            else:
                st.markdown(
                    f"<div style='font-size:0.68rem;color:var(--text-3);font-family:\"DM Mono\",monospace;"f"padding-top:0.55rem'>{_yr_from} → {_yr_to}</div>",
                    unsafe_allow_html=True,
                )

        # ── Granularity → resample rule ─────────────────────────────────────
        _resample_map = {
            "Monthly":     "ME",
            "Quarterly":   "QE",
            "Half-Yearly": "6ME",
        ...
        _rule = _resample_map[_granularity]

        # ── Filter data to selected window ──────────────────────────────────
        _date_from = pd.Timestamp(f"{_yr_from}-01-01")
        _date_to   = pd.Timestamp(f"{_yr_to}-12-31")

        _comp_free_w = comp_free[
            (comp_free["review_date"] >= _date_from) &
            (comp_free["review_date"] <= _date_to)
        ].copy()
        _comp_all_w  = comp_all[
            (comp_all["review_date"] >= _date_from) &
            (comp_all["review_date"] <= _date_to)
        ].copy()

        # ── Resample ────────────────────────────────────────────────────────
        trend_df = (
            _comp_free_w.set_index("review_date")["combined_sentiment"]
            .resample(_rule).mean()
            .dropna()
            .reset_index()
        )
        ...
        rating_trend = (
            _comp_all_w.set_index("review_date")["overall_rating"]
            .resample(_rule).mean()
            .dropna()
            .reset_index()
        )
        ...
        vol_trend = (
            _comp_all_w.set_index("review_date")["overall_rating"]
            .resample(_rule).count()
            .reset_index()
        )
        vol_trend.columns = ["date", "count"]

        # ── Stats strip ─────────────────────────────────────────────────────
        if len(trend_df) >= 2:
            _t_mean  = trend_df["sentiment"].mean()
            _t_delta = trend_df["sentiment"].iloc[-1] - trend_df["sentiment"].iloc[0]
            _t_vol   = trend_df["sentiment"].std()
            _t_dir   = "▲ Improving" if _t_delta > 0.01 else ("▼ Declining" if _t_delta < -0.01 else "→ Stable")
            _t_dir_c = "#059669" if _t_delta > 0.01 else ("#dc2626" if _t_delta < -0.01 else "#d97706")
            _n_pts   = len(trend_df)

            st.markdown(f"""
            <div style='display:flex;gap:0.55rem;margin-bottom:0.65rem;flex-wrap:wrap'>
                <div style='background:rgba(5,150,105,0.06);border:1px solid rgba(5,150,105,0.14);border-radius:var(--r-sm);padding:0.38rem 0.75rem;display:flex;align-items:center;gap:0.45rem'>
                    <span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.09em;color:var(--text-3);font-family:"DM Mono",monospace'>Avg Sentiment</span>
                    <span style='font-size:0.85rem;font-family:"DM Serif Display",serif;color:{"#059669" if _t_mean >= 0 else "#dc2626"}'>{_t_mean:+.3f}</span>
                </div>
                <div style='background:rgba(5,150,105,0.06);border:1px solid rgba(5,150,105,0.14);border-radius:var(--r-sm);padding:0.38rem 0.75rem;display:flex;align-items:center;gap:0.45rem'>
                    <span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.09em;color:var(--text-3);font-family:"DM Mono",monospace'>Trend</span>
                    <span style='font-size:0.8rem;font-weight:600;color:{_t_dir_c};font-family:"DM Sans",sans-serif'>{_t_dir}</span>
                </div>
                <div style='background:rgba(5,150,105,0.06);border:1px solid rgba(5,150,105,0.14);border-radius:var(--r-sm);padding:0.38rem 0.75rem;display:flex;align-items:center;gap:0.45rem'>
                    <span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.09em;color:var(--text-3);font-family:"DM Mono",monospace'>Δ</span>
                    <span style='font-size:0.85rem;font-family:"DM Serif Display",serif;color:{"#059669" if _t_delta >= 0 else "#dc2626"}'>{_t_delta:+.3f}</span>
                </div>
                <div style='background:rgba(5,150,105,0.06);border:1px solid rgba(5,150,105,0.14);border-radius:var(--r-sm);padding:0.38rem 0.75rem;display:flex;align-items:center;gap:0.45rem'>
                    <span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.09em;color:var(--text-3);font-family:"DM Mono",monospace'>Volatility</span>
                    <span style='font-size:0.85rem;font-family:"DM Serif Display",serif;color:var(--text)'>{_t_vol:.3f}</span>
                </div>
                <div style='background:rgba(5,150,105,0.06);border:1px solid rgba(5,150,105,0.14);border-radius:var(--r-sm);padding:0.38rem 0.75rem;display:flex;align-items:center;gap:0.45rem'>
                    <span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.09em;color:var(--text-3);font-family:"DM Mono",monospace'>Periods</span>
                    <span style='font-size:0.85rem;font-family:"DM Serif Display",serif;color:var(--text)'>{_n_pts}</span>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        if len(trend_df) >= 2:
            ...
            # Positive / negative fill split
            pos_mask = trend_df["sentiment"] >= 0
            neg_mask = trend_df["sentiment"] < 0

            # Full sentiment line with fill
            fig_t.add_trace(go.Scatter(
                x=trend_df["date"], y=trend_df["sentiment"],
                mode="lines+markers",
                line=dict(color=color, width=2.5, shape="spline"),
                marker=dict(size=7, color=trend_df["sentiment"].apply(
                    lambda v: "#059669" if v >= 0 else "#dc2626"),
                    line=dict(color=PLOT_BG, width=2)),
                fill="tozeroy",
                fillcolor=hex_to_rgba(color, 0.09),
                hovertemplate="<b>%{x|%b %Y}</b><br>Sentiment: <b>%{y:+.3f}</b><extra></extra>",
            ...
            if len(trend_df) > 3:
                _x_num = np.arange(len(trend_df))
                _slope, _intercept, *_ = stats.linregress(_x_num, trend_df["sentiment"])
                _trend_y = _slope * _x_num + _intercept
                fig_t.add_trace(go.Scatter(
                    x=trend_df["date"], y=_trend_y,
                    mode="lines",
                    line=dict(color=color, width=1.2, dash="longdash"),
                    opacity=0.35,
                    hoverinfo="skip",
                    showlegend=False,
                    yaxis="y1",
            ...
            if len(trend_df) > 2:
                ...
                for idx, symbol, yshift, ann_clr, ann_border in [
                    (peak_idx,   "▲ Peak",   0.03,  "#059669", "rgba(5,150,105,0.35)"),
                    (trough_idx, "▼ Trough", -0.05, "#dc2626", "rgba(220,38,38,0.35)"),
                ]:
                    fig_t.add_annotation(
                        x=trend_df.loc[idx, "date"],
                        y=trend_df.loc[idx, "sentiment"] + yshift,
                        text=f"<b>{symbol}</b> {trend_df.loc[idx,'sentiment']:+.2f}",
                        showarrow=True, arrowhead=2, arrowcolor=ann_clr,
                        arrowsize=0.8, arrowwidth=1.2, ax=0, ay=-22,
                        font=dict(size=8.5, color=ann_clr, family="DM Mono"),
            ...
            # ── Overlay ────────────────────────────────────────────────────
            if _overlay == "Avg Rating" and len(rating_trend) >= 2:
                fig_t.add_trace(go.Scatter(
                    ...
                    hovertemplate="<b>%{x|%b %Y}</b><br>Avg Rating: <b>%{y:.2f}★</b><extra></extra>",
            ...
            elif _overlay == "Review Volume" and len(vol_trend) >= 2:
                fig_t.add_trace(go.Bar(
                    x=vol_trend["date"], y=vol_trend["count"],
                    marker=dict(color=color, opacity=0.13),
                    hovertemplate="<b>%{x|%b %Y}</b><br>Reviews: <b>%{y}</b><extra></extra>",
                    name="Review Volume (R)", yaxis="y2",
                ))
                _y2_axis = dict(
                    title=dict(text="Review Count",
                               font=dict(color=color, size=10.5, family="DM Sans")),
                    overlaying="y", side="right",
                    tickfont=dict(color=color, size=10, family="DM Mono"),
                    showgrid=False, zeroline=False,
                )
            else:
                _y2_axis = dict(visible=False, overlaying="y", side="right")

            _x_label = {"Quarterly": "Quarter", "Monthly": "Month",
                        "Half-Yearly": "Half-Year", "Yearly": "Year"}[_granularity]

            fig_t.update_layout(
                **base_layout(
                    xaxis=ax(showgrid=False, title=dict(text=_x_label, font=TITLE_FONT)),
                    yaxis=dict(
                    ...
                    margin=dict(l=15, r=65, t=10, b=62),
                    height=310,
                )
            )
            ...
            if len(trend_df) >= 3:
                trend_df["prev"]   = trend_df["sentiment"].shift(1)
                trend_df["change"] = trend_df["sentiment"] - trend_df["prev"]
                _recent = trend_df.dropna(subset=["change"]).tail(4)
                if len(_recent) > 0:
                    _period_label = {"QE":"Q","ME":"Mo","6ME":"H","YE":"Y"}[_rule]
                    _change_cells = ""
                    for _, _pr in _recent.iterrows():
                        _c = _pr["change"]
                        _clr_c = "#059669" if _c >= 0 else "#dc2626"
                        _arrow = "▲" if _c >= 0 else "▼"
                        _change_cells += (
                            f"<td style='text-align:center;padding:0.35rem 0.6rem'>"
                            f"<div style='font-size:0.62rem;color:var(--text-3);"f"font-family:\"DM Mono\",monospace'>{_pr['date'].strftime('%b %Y')}</div>"
                            f"<div style='font-size:0.8rem;font-family:\"DM Serif Display\",serif;"f"color:{_clr_c}'>{_arrow}{abs(_c):.3f}</div>"
                            f"</td>"
                        )
                    st.markdown(f"""
                    <div style='margin-top:-0.2rem;border:1px solid var(--border);border-radius:var(--r-sm);background:var(--surface);overflow:hidden'>
                        <div style='padding:0.3rem 0.6rem;background:rgba(5,150,105,0.04);border-bottom:1px solid var(--border);font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:var(--text-3);font-family:"DM Mono",monospace'>
        ...
        elif len(_comp_free_w) == 0:
            st.markdown(f"""
            <div style='padding:1.2rem;background:rgba(180,83,9,0.05);border:1px solid rgba(180,83,9,0.18);border-radius:var(--r-sm);font-size:0.8rem;color:#92400e;font-family:"DM Sans",sans-serif'>
                ⚠ No reviews found for <b>{sel}</b> in the selected window
                (<b>{_yr_from}–{_yr_to}</b>). Try expanding the range.
            </div>
            """, unsafe_allow_html=True)
        else:
            st.info("Not enough data points in this window — try a wider range or coarser granularity.")
    ...
    with phrase_col:
            ...
            if like_phrases:
                ...
                for word, count in like_phrases:
                    ...
                    <div style='display:flex;align-items:center;gap:0.65rem;margin-bottom:0.52rem'>
                        <span style='font-size:0.78rem;color:var(--text);width:115px;flex-shrink:0;font-family:"DM Sans",sans-serif;font-weight:500'>{word}</span>
                        <div style='flex:1;background:rgba(5,150,105,0.05);border-radius:999px;height:6px;overflow:hidden;border:1px solid rgba(5,150,105,0.07)'>
                            <div style='background:#059669;width:{pct:.0f}%;height:6px;border-radius:999px;opacity:0.78'></div>
                        </div>
                        <span style='font-size:0.68rem;color:var(--text-3);width:24px;text-align:right;font-family:"DM Mono",monospace'>{count}</span>
                    </div>""", unsafe_allow_html=True)
            else:
        ...
        with tab2:
            if dislike_phrases:
                ...
                for word, count in dislike_phrases:
                    ...
                    <div style='display:flex;align-items:center;gap:0.65rem;margin-bottom:0.52rem'>
                        <span style='font-size:0.78rem;color:var(--text);width:115px;flex-shrink:0;font-family:"DM Sans",sans-serif;font-weight:500'>{word}</span>
                        <div style='flex:1;background:rgba(248,113,113,0.05);border-radius:999px;height:6px;overflow:hidden;border:1px solid rgba(248,113,113,0.07)'>
                            <div style='background:#dc2626;width:{pct:.0f}%;height:6px;border-radius:999px;opacity:0.78'></div>
                        </div>
                        <span style='font-size:0.68rem;color:var(--text-3);width:24px;text-align:right;font-family:"DM Mono",monospace'>{count}</span>
                    </div>""", unsafe_allow_html=True)
            else:
        ...
        st.markdown('<div class="sec-title" style="font-size:1rem">Theme Keyword Signal</div>',
                    unsafe_allow_html=True)
        st.markdown("""
        <div style='font-size:0.75rem;color:var(--text-3);margin-bottom:0.85rem;font-family:"DM Sans",sans-serif;font-weight:300;line-height:1.6'>
            Keyword frequency across like &amp; dislike text, ranked by presence per theme.
            Bar width = share of max count within that polarity column.
        </div>""", unsafe_allow_html=True)
        ...
        # Build full keyword data per theme
        kw_data = []
        for t_col, t_lbl in zip(THEME_COLS, THEME_LABELS):
            meta_k = THEME_META[t_col]
            pos_counts = {kw: like_text_all.count(kw) for kw in THEME_KW_POS[t_col]}
            neg_counts = {kw: dislike_text_all.count(kw) for kw in THEME_KW_NEG[t_col]}
            # Top 3 of each
            top_pos = sorted(pos_counts.items(), key=lambda x: x[1], reverse=True)[:3]
            top_neg = sorted(neg_counts.items(), key=lambda x: x[1], reverse=True)[:3]
            total_pos = sum(pos_counts.values())
            total_neg = sum(neg_counts.values())
            net = total_pos - total_neg
            kw_data.append({
                "col": t_col, "label": t_lbl, "meta": meta_k,
                "top_pos": top_pos, "top_neg": top_neg,
                "total_pos": total_pos, "total_neg": total_neg, "net": net,
            })

        # Global max for bar normalisation
        global_pos_max = max((d["top_pos"][0][1] if d["top_pos"] else 1) for d in kw_data) or 1
        global_neg_max = max((d["top_neg"][0][1] if d["top_neg"] else 1) for d in kw_data) or 1

        for d in kw_data:
            meta_k    = d["meta"]
            net       = d["net"]
            net_color = "#059669" if net > 0 else ("#dc2626" if net < 0 else "var(--text-3)")
            net_arrow = "▲" if net > 0 else ("▼" if net < 0 else "—")
            net_label = f"{net_arrow} {abs(net):+d}" if net != 0 else "— 0"

            # Build positive keyword bar rows
            def _bar_row_pos(kw, cnt, max_cnt):
                bw = round(cnt / max_cnt * 100)
                return (f"<div style='display:flex;align-items:center;gap:0.45rem;margin-bottom:0.28rem'>"
                        f"<span style='font-size:0.67rem;font-family:DM Mono,monospace;color:#059669;width:74px;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{kw}</span>"
                        f"<div style='flex:1;background:rgba(5,150,105,0.08);border-radius:999px;height:5px;overflow:hidden;min-width:40px'>"
                        f"<div style='background:#059669;width:{bw}%;height:5px;border-radius:999px;opacity:0.82'></div></div>"
                        f"<span style='font-size:0.63rem;color:#9ca3af;font-family:DM Mono,monospace;width:20px;text-align:right;flex-shrink:0'>{cnt}</span>"
                        f"</div>")

            def _bar_row_neg(kw, cnt, max_cnt):
                bw = round(cnt / max_cnt * 100)
                return (f"<div style='display:flex;align-items:center;gap:0.45rem;margin-bottom:0.28rem'>"
                        f"<span style='font-size:0.67rem;font-family:DM Mono,monospace;color:#dc2626;width:74px;flex-shrink:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis'>{kw}</span>"
                        f"<div style='flex:1;background:rgba(220,38,38,0.08);border-radius:999px;height:5px;overflow:hidden;min-width:40px'>"
                        f"<div style='background:#dc2626;width:{bw}%;height:5px;border-radius:999px;opacity:0.82'></div></div>"
                        f"<span style='font-size:0.63rem;color:#9ca3af;font-family:DM Mono,monospace;width:20px;text-align:right;flex-shrink:0'>{cnt}</span>"
                        f"</div>")

            no_data = "<span style='font-size:0.67rem;color:#9ca3af;font-family:DM Mono,monospace'>no data</span>"
            pos_bars_html = "".join(_bar_row_pos(kw, cnt, global_pos_max) for kw, cnt in d["top_pos"] if cnt > 0) or no_data
            neg_bars_html = "".join(_bar_row_neg(kw, cnt, global_neg_max) for kw, cnt in d["top_neg"] if cnt > 0) or no_data

            card_html = (
                f"<div style='background:var(--surface);border:1px solid var(--border);border-left:3px solid {meta_k['color']};border-radius:var(--r-md);padding:0.75rem 0.9rem;margin-bottom:0.55rem;box-shadow:var(--shadow-sm)'>"
                f"<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:0.6rem'>"
                f"<span style='font-size:0.75rem;font-weight:600;color:{meta_k['color']};font-family:DM Sans,sans-serif'>{meta_k['icon']} {d['label']}</span>"
                f"<div style='display:flex;align-items:center;gap:0.5rem'>"
                f"<span style='font-size:0.6rem;text-transform:uppercase;letter-spacing:0.08em;color:var(--text-3);font-family:DM Mono,monospace'>Net</span>"
                f"<span style='font-size:0.8rem;font-weight:700;color:{net_color};font-family:DM Mono,monospace'>{net_label}</span>"
                f"</div></div>"
                f"<div style='display:grid;grid-template-columns:1fr 1fr;gap:0.65rem'>"
                f"<div>"
                f"<div style='font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:#059669;font-family:DM Mono,monospace;margin-bottom:0.35rem;display:flex;align-items:center;gap:0.3rem'>"
                f"<span style='width:6px;height:6px;border-radius:50%;background:#059669;display:inline-block'></span>"
                f"Positive &middot; {d['total_pos']}</div>"
                f"{pos_bars_html}"
                f"</div>"
                f"<div>"
                f"<div style='font-size:0.6rem;font-weight:700;text-transform:uppercase;letter-spacing:0.09em;color:#dc2626;font-family:DM Mono,monospace;margin-bottom:0.35rem;display:flex;align-items:center;gap:0.3rem'>"
                f"<span style='width:6px;height:6px;border-radius:50%;background:#dc2626;display:inline-block'></span>"
                f"Negative &middot; {d['total_neg']}</div>"
                f"{neg_bars_html}"
                f"</div>"
                f"</div></div>"
            )
        ...
        if len(sample_pool) > 0:
            ...
            for _, rev in sample_reviews.iterrows():
                ...
                if pd.notna(l_sent) and pd.notna(d_sent):
                    sent_badge = (
                        f"<span style='font-size:0.63rem;color:#059669;font-weight:600;font-family:\"DM Mono\",monospace'>L:{l_sent:+.2f}</span>"
                        f"<span style='font-size:0.63rem;color:#dc2626;font-weight:600;font-family:\"DM Mono\",monospace;margin-left:0.4rem'>D:{d_sent:+.2f}</span>"
                    )
                rev_html = (
                    f"<div class='rev-card'>"
                    f"<div style='display:flex;justify-content:space-between;margin-bottom:0.38rem;align-items:center'>"
                    f"<span style='font-size:0.78rem;color:{star_clr};font-family:\"DM Mono\",monospace;letter-spacing:0.05em'>{stars_str}</span>"
                    f"<div style='display:flex;gap:0.55rem;align-items:center'>"
                    f"{sent_badge}"
                    f"<span style='font-size:0.67rem;color:var(--text-3);font-family:\"DM Mono\",monospace'>{date}</span>"
                    f"</div></div>"
                    f"<div style='font-size:0.76rem;color:var(--text-2);margin-bottom:0.28rem;font-family:\"DM Sans\",sans-serif;line-height:1.6;font-weight:300'>"
                    f"<span style='color:#059669;font-weight:600;margin-right:0.3rem'>&#8593;</span>{like}"
                    f"</div>"
                    f"<div style='font-size:0.76rem;color:var(--text-2);font-family:\"DM Sans\",sans-serif;line-height:1.6;font-weight:300'>"
                    f"<span style='color:#dc2626;font-weight:600;margin-right:0.3rem'>&#8595;</span>{dis}"
                    f"</div></div>"
                )
    ...
    if sel in all_means.index:
        ...
        prior_cutoff  = comp_all["review_date"].max() - pd.DateOffset(years=4)
        n_recent = len(comp_all[comp_all["review_date"] >= recent_cutoff])
        # Compare same-length window: 2 years prior to the recent window
        n_prior  = len(comp_all[(comp_all["review_date"] >= prior_cutoff) &
                                (comp_all["review_date"] <  recent_cutoff)])
        ...
        for col, icon, title, rows in signal_data:
            with col:
                rows_html = "".join(
                    f"<div style='display:flex;justify-content:space-between;align-items:baseline;padding:0.38rem 0;border-bottom:1px solid rgba(5,150,105,0.06)'>"
                    f"<span style='font-size:0.74rem;color:var(--text-3);font-family:\"DM Sans\",sans-serif'>{lbl}</span>"
                    f"<div style='text-align:right'>"
                    f"<span style='font-size:0.78rem;font-weight:600;color:{clr};font-family:\"DM Mono\",monospace'>{val}</span>"
                    + (f"<span style='font-size:0.65rem;color:var(--text-3);margin-left:0.3rem'>{sub}</span>" if sub else "")
                    + f"</div></div>"
                    for lbl, val, clr, sub in rows
                )
                card_html = (
                    f"<div class='ins-card' style='border-top:2px solid rgba(5,150,105,0.2)'>"
                    f"<div style='display:flex;align-items:center;gap:0.5rem;margin-bottom:0.8rem'>"
                    f"<div style='width:30px;height:30px;border-radius:8px;background:rgba(5,150,105,0.1);border:1px solid rgba(5,150,105,0.2);display:flex;align-items:center;justify-content:center;font-size:0.95rem'>{icon}</div>"
                    f"<span style='font-weight:600;color:var(--text);font-size:0.85rem;font-family:\"DM Sans\",sans-serif'>{title}</span>"
                    f"</div>"
                    f"{rows_html}"
                    f"</div>"
                )
    ...
    # Count rows that are usable: NOT structured AND NOT low_signal
    # Using boolean masks avoids double-counting rows that satisfy both exclusion criteria
    if "structured_text" in comp_all.columns and "low_signal" in comp_all.columns:
        _excl_mask = comp_all["structured_text"].astype(bool) | comp_all["low_signal"].astype(bool)
        n_usable = int((~_excl_mask).sum())
    else:
