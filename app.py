...
/* ── Streamlit expander — professional look ── */
...
[data-testid="stExpander"] summary:hover {
    background: rgba(13,122,72,0.04) !important;
}
...
/* ── Professional select company bar ── */
.company-selector {
    background: var(--surface);
    border: 1px solid var(--border);
    border-radius: var(--r-md);
                ...
                <div style='display:flex;justify-content:space-between;align-items:center;
                    ...
                    <div style='display:flex;align-items:center;gap:0.55rem'>
                        <span style='font-size:0.68rem;font-family:"DM Mono",monospace;
                                     ...
                                     padding:0.1rem 0.45rem;border-radius:4px'>#{rank}</span>
        ...
        with col:
            ...
            <div class='ins-card' style='border-top:2px solid {clr}33'>
                <div style='display:flex;align-items:center;gap:0.6rem;margin-bottom:0.65rem'>
                    ...
                    <div style='font-weight:600;color:{clr};font-size:0.78rem;
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    <div class='company-selector'>
        <span class='company-selector-label'>Select Company</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    caveat_note = (
        ...
        f"Based on only {n_reviews} reviews — treat with caution.</span>"
    ...
    verdict_html = (
        ...
        f"font-family:\"DM Sans\",sans-serif'>Rank #{rank} — {vname}{caveat}</span>"
        f"<span style='color:var(--text-3);font-size:0.75rem;font-family:\"DM Sans\",sans-serif;font-weight:400'> · </span>"
            ...
            <div style='margin-bottom:1.3rem'>
                ...
                <div style='background:rgba(5,150,105,0.05);border-radius:999px;height:10px;
                    ...
                    <div style='background:linear-gradient(90deg,{clr}99,{clr});
                                ...
                                margin-left:{midpoint if val < 0 else bpct_disp if bpct_disp > midpoint else 0:.0f}%;
                                opacity:0.88'></div>
                </div>
    ...
    with st.expander("Sentiment Polarity Distribution", expanded=False):
        ...
        if len(sent_vals) > 5:
            ...
            if is_bimodal:
                ...
                <div style='background:rgba(220,38,38,0.05);border:1px solid rgba(220,38,38,0.18);
                    ...
                    <b>Split workforce</b> — average sentiment hides disagreement.
    ...
    with st.expander("WCI Theme Contribution Breakdown", expanded=False):
     st.markdown("""
     ...
     if sel in all_means.index:
        ...
        with tc2:
            ...
            for i, tc in enumerate(sorted(theme_contribs, key=lambda x: x["contribution"], reverse=True)):
                vs_avg = tc["contribution"] - all_contribs_mean.get(THEME_COLS[theme_contribs.index(tc)], 0)
    ...
    with phrase_col:
        ...
        st.markdown("<div style='height:1rem'></div>", unsafe_allow_html=True)
        with st.expander("Theme Keyword Signal", expanded=False):
         THEME_KW_POS = {
    ...
    with reviews_col:
        ...
        sample_pool = (comp_free[~comp_free[low_col]] if low_col else comp_free).dropna(
        ...
        if len(sample_pool) > 0:
            ...
            for _, rev in sample_reviews.iterrows():
                ...
                like = str(rev["like_text"])[:145] + ("…" if len(str(rev["like_text"])) > 145 else "")
                dis  = str(rev["dislike_text"])[:145] + ("…" if len(str(rev["dislike_text"])) > 145 else "")
                ...
                rev_html = (
                    ...
                    "font-family:\'DM Mono\',monospace;letter-spacing:0.05em'>" + stars_str + "</span>"
        ...
        for col, icon, title, rows in signal_data:
            with col:
                ...
                for lbl, val, clr, sub in rows:
                    sub_html = ("<span style=\"font-size:0.65rem;color:var(--text-3);margin-left:0.3rem\">" + sub + "</span>") if sub else ""
                    rows_html += (
                ...
                card_html = (
                    ...
                    "<span style='font-weight:600;color:var(--text);font-size:0.82rem;"
                    "font-family:\"DM Sans\",sans-serif;text-transform:uppercase;letter-spacing:0.04em'>" + title + "</span>"
                    "</div>" + rows_html + "</div>"
                )
    ...
    with insight_col:
        ...
        for icon, clr, title, body in interp_cards:
            ...
            <div class='ins-card' style='margin-bottom:0.7rem;border-top:2px solid {clr}33'>
                <div style='display:flex;align-items:center;gap:0.6rem;margin-bottom:0.55rem'>
                    <div style='width:6px;height:6px;border-radius:50%;background:{clr};flex-shrink:0'></div>
                    <span style='font-weight:600;color:{clr};font-size:0.78rem;
